...
-----------------------------#>
        ...
        if ($tpschInitial -eq $true){Write-host "$schInitial2 is present"}else{write-host "$schInitial2 is missing" -ForegroundColor Red | Pause }
    
        $schOUDeploy = "$($Pwdir)\$($ouDeployment)"
        $tpschOUDeploy = Test-Path $schOUDeploy
        if ($tpschOUDeploy  -eq $true){Write-host "$schOUDeploy is present"}else{write-host "$schOUDeploy is missing" -ForegroundColor Red | Pause }
